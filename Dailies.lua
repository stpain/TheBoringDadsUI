...
function TheBoringDad_DailiesMixin:Database_OnInitialised()
    ...
    local db = Database:FetchTable("dailies");
    if db == false then
        local dbObj = {
            name = "dailies",
            db = {
        ...
        Database:RegisterTable(dbObj)
        self.db = Database:FetchTable("dailies");
    ...
    addon:TriggerEvent("Module_OnLoad", self)
...
function TheBoringDads_DailiesListviewItemMixin:SetDataBinding(binding, height)
    ...
    --if this is a quest do fancy stuff
        ...
        self.completed:Show()
        self.completed.label:SetText(binding.info.link)
        self.header:Hide()
