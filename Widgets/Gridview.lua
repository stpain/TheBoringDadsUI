...
TheBoringdadsGridviewMixin = {}
function TheBoringdadsGridviewMixin:OnLoad()
    self.data = {}
    self.frames = {}
    self.itemMinWidth = 0
    self.itemMaxWidth = 0
...
function TheBoringdadsGridviewMixin:InitFramePool(type, template)
    self.framePool = CreateFramePool(type, self, template);
end

function TheBoringdadsGridviewMixin:SetMinMaxSize(min, max)
    self.itemMinWidth = min;
    self.itemMaxWidth = max;
end

function TheBoringdadsGridviewMixin:Insert(info)
...
function TheBoringdadsGridviewMixin:RemoveFrame(frame)
    local key;
...
function TheBoringdadsGridviewMixin:InsertTable(tbl)

end

function TheBoringdadsGridviewMixin:Flush()
    self.data = {}
    for k, f in ipairs(self.frames) do
        if f.ResetDataBinding then
            f:ResetDataBinding()
...
function TheBoringdadsGridviewMixin:GetItemSize()
...
function TheBoringdadsGridviewMixin:UpdateLayout()
    self:GetItemSize()
...
function TheBoringdadsGridviewMixin:GetFrames()
    return self.frames;
end